window.addEventListener('load', function(){
    ...
    var itensAutoScroll = document.querySelectorAll('.item-auto-scroll')
    const menu = new Menu()

    //CLASSE DE MANIPULAÇÃO DO MENU
    function Menu(){
        this.itens          = document.querySelectorAll('.ul-items-menu .item')
        this.btnsSubmenus   = document.querySelectorAll(('.ul-items-menu .content-submenu .btnSubmenu'))
    ...
    function abrirMenu(menuAberto){
        ...
        if(menuAberto){
            ...
            contentMenu.parentNode.classList.add('menu-aberto')
            ...
            contentMenu.parentNode.classList.remove('menu-aberto')
    ...
    function autoScrollMenu(itemAutoScroll){

        let seletor = itemAutoScroll.querySelector('a').getAttribute("href")
        ...
        //Verifica se o itemAutoScroll é um item do menu
        let itemMenu = itemAutoScroll.getAttribute("itemMenu")
        
        //Verifica se a tela ta mobile | caso sim fecha o menu n ação do scroll
        if(window.matchMedia("(max-width:990px)").matches && itemMenu === "true"){
